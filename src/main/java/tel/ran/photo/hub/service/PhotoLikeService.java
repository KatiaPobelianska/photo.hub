...
public class PhotoLikeService {
    ...
    public void save(PhotoLike photoLike){
        Optional<PhotoLike> photoLikeOptional = jdbcTemplate.query(
                "select * from photo_like where post_id=? and person_username=?", new Object[]{photoLike.getPost().getId(), photoLike.getPerson().getUsername() }, new BeanPropertyRowMapper<>(PhotoLike.class)
        ).stream().findAny();
